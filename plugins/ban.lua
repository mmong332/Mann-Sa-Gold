      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "ادفر" then
         if member_id == from_id then
            send_large_msg(receiver, "متكدر تطرد نفسك غبي😂💔")
			return
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "متكدر تطرد الادمنيه والمدراء حمبي🌚💔")
			return
         end
		 kick_user(member_id, chat_id)
      elseif get_cmd == 'حظر' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
			send_large_msg(receiver, "متكدر تحظر الادمنيه والمدراء غبي🌚💔")
			return
        end
        send_large_msg(receiver, 'العضو['..member_id..']  @'..member..'   اوك انحظر🐸💔')
		ban_user(member_id, chat_id)
      elseif get_cmd == 'الغاء الحظر' then
        send_large_msg(receiver, 'العضو ['..member_id..'] @'..member..'  لغيت الحظر مالته☺️💔')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        	return 'العضو'..user_id..'وخرت حظره وبعد لتعيدهة🌚💔'
      elseif get_cmd == 'حظر عام' then
        send_large_msg(receiver, 'العضو ['..member_id..'] @'..member..'    اوك حظرته عام🐸💔')
		banall_user(member_id)
      elseif get_cmd == 'الغاء العام' then
        send_large_msg(receiver, 'العضو['..member_id..'] @'..member..'  لغيت الحظره العام منه☺️💔')
...
local support_id = msg.from.id
 if matches[1]:lower() == 'ايدي' and msg.to.type == "chat" or msg.to.type == "user" then
    if msg.to.type == "user" then
      return "ايدي مال البوت😊📍 : "..msg.to.id.. "\n\n🌚💔 الايدي مالتك: "..msg.from.id.. "\n\n🕵 المطور@Sudo_Sky"
    end
    ...
    elseif matches[1]:lower() == 'ايدي' then
      ...
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "👥 ايدي الكروب" ..string.gsub(msg.to.print_name, "_", " ").. ":\n\n"..msg.to.id
    end
  end
  ...
  if matches[1]:lower() == 'حظر' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "متكدر تحظر الادمنيه والمدراء حمبي🐸💔"
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return "متكدر تطرد نفسك حمبي🌚💔"
        end
        local print_name = user_print_name(msg.from):gsub("‮", "")
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
		...
		send_large_msg(receiver, 'العضو ['..matches[2]..']  اوك حظرته من الكروب🌚💔')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'حظر',
  ...
  if matches[1]:lower() == 'الغاء الحظر' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	return 'العضو '..user_id..' وخرت حظره بعد لتعيدهة🌚💔'
      else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'الغاء الحظر',
...
if matches[1]:lower() == 'دي' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "متكدر تطرد الادمنيه والمدراء حمبي🐸💔"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "متكدر تطرد نفسك حمبي🌚💔"
		end
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'العضو ['..user_id..' ] اوك حظرته عام🌚💔'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'حظر عام',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'العضو '..user_id..' لغيت حظره بعد لتعيدهة🌚💔'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'الغاء العام',
